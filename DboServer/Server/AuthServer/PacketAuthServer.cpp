...
void CClientSession::SendCharLogInReq(CNtlPacket* pPacket, CAuthServer* app)
{
    ...
    if (req->wLVersion == CLIENT_LVER && (int)req->wRVersion == CLIENT_RVER)
    {
        ...
        if (pCharServer == NULL || !pCharServer->bIsOn)
    ...
    if (resultcode == AUTH_SUCCESS)
    {
        ...
        sprintf_s(mac, 128, "%02X:%02X:%02X:%02X:%02X:%02X",
            req->abyMacAddress[0], req->abyMacAddress[1], req->abyMacAddress[2],
            req->abyMacAddress[3], req->abyMacAddress[4], req->abyMacAddress[5]);
        ...
        smart_ptr<QueryResult> result = GetAccDB.Query(
            "SELECT AccountID,Password_hash,acc_status,isGm,lastServerFarmId,founder "
            "FROM accounts WHERE Username = \"%s\" LIMIT 1",
            GetAccDB.EscapeString(username).c_str());

        if (result)
        {
            Field* fields = result->Fetch();
            if (0 != strcmp(fields[1].GetString(), hashedPassword)) 
            {
            ...
            else
            {
                ...
                // server locked?
                if (app->GetDisableConnection() == TRUE)
                {
                    if (app->GetFounderConnection() == TRUE)
                    {
                        if (fields[5].GetINT() == 0 && isGm == false)
                ...
                if (/*accstatus == "pending" ||*/ accstatus == "block")
                ...
                if (resultcode == AUTH_SUCCESS)
                {
                    ...
                    if (app->AddPlayer(this->AccountID, this) == true)
                    {
                        ...
                        res->bIsGM = isGm;
                        res->dwAllowedFunctionForDeveloper =
                            DBO_ALLOWED_FUNC_FOR_DEV_FLAG_HUMAN |
                            DBO_ALLOWED_FUNC_FOR_DEV_FLAG_NAMEK |
                            DBO_ALLOWED_FUNC_FOR_DEV_FLAG_MAJIN;
    ...
    if (resultcode != AUTH_SUCCESS)
    {
        ...
        if (m_byLoginTrys >= 5)
        {
            ERR_LOG(LOG_SYSTEM, "Session %u, IP %s, User %s too many login fails. IP temp blocked (session) \n",
