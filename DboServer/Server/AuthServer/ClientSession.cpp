...
int CClientSession::OnAccept()
{
    this->AccountID = 0;
    m_byLoginTrys = 0;
    m_createTick = GetTickCount64();
    m_lastActivityTick = m_createTick;
...
{
    // cada paquete recibido = refresco actividad
    RefreshActivity();
    ...
    {
        unsigned char buf[] = {
            0x10, 0x00, 0x84, 0xfb, 0x48, 0xf4, 0x8e, 0x5a,
            0xb6, 0x67, 0xe2, 0x3d, 0x6e, 0x14, 0xb4, 0xa3,
            0xc3, 0x24, 0x9e, 0x5f, 0xe3, 0xd1, 0xd5, 0x88,
            0x10, 0x0d, 0x68, 0x4f, 0x3b, 0xa5, 0xed, 0x37,
            0xed, 0x4a
        };
    ...
    case UA_LOGIN_REQ:
    case UA_LOGIN_TW_REQ:
        SendCharLogInReq(pPacket, app);
        // si login OK adentro -> llamar SetAuthenticated();
        break;

    case UA_LOGIN_CREATEUSER_REQ:
    case UA_LOGIN_DISCONNECT_REQ:
        SendLoginDcReq(pPacket, app);
        break;
